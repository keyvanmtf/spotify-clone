...
import 'package:spotify_clone/features/song%20player/presentaion/bloc/song_player_cubit.dart';
    ...
    return Scaffold(
      ...
      body: BlocProvider(
        create: (context) => SongPlayerCubit()
          ..loadSong(
              "${imageUrl.songfireStorage}${songEntity.title}.mp3?${imageUrl.mediaAlt}"),
    ...
    return Container(
      ...
      decoration: BoxDecoration(
          ...
          image: DecorationImage(
            fit: BoxFit.cover,
            image: NetworkImage(
                "${imageUrl.coverfireStorage}${songEntity.title}_${songEntity.artist}.jpg?${imageUrl.mediaAlt}"),
          )),
    );
  }
  ...
  Widget _songPlayer(BuildContext context) {
    return BlocBuilder<SongPlayerCubit, SongPlayerState>(
      builder: (context, state) {
        if (state is SongPlayeLoading) {
          return const CircularProgressIndicator();
        }
        if (state is SongPlayeLoaded) {
          return Column(
            children: [
              Slider(
                  value: context
